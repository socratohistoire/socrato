...
type TestRequest = { questionId: string; content: string };
...
export async function analyzeActeUnionTestResponse(request: TestRequest) {
  ...
  try {
    ...
    const response: StudentResponse = {
      ...
      documentIds: [...definition.documentIds], attemptNumber: 1, hintLevel: 0, content: request.content,
    };
    ...
    const feedback = createPedagogicalFeedback(analysis, definition, analysis.pedagogicalOutcome === "non_exploitable" ? 1 : 0);
