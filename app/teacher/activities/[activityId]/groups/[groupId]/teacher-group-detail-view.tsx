...
function StudentConsolidationAction({ groupId, student }: { groupId: string; student: TeacherGroupStudent }) {
  const [open, setOpen] = useState(false);
  const displayLabel = student.displayLabel.replace(/\s*\([^)]*\)$/, "");
  const revisionParams = new URLSearchParams({ consolidationStudent: student.id, consolidationGroup: groupId });
  const guidanceParams = new URLSearchParams({ student: student.id, group: groupId });
  useEffect(() => {
    if (!open) return;
  ...
  return <>
    <button type="button" className="group-student-consolidation" aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen(true)} aria-label={`Assigner une activité à ${displayLabel}`}>Assigner une activité <span aria-hidden="true">→</span></button>
    {open ? <div className="student-assignment-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="student-assignment-dialog" role="dialog" aria-modal="true" aria-labelledby={`assignment-title-${student.id}`}>
        <button type="button" className="student-assignment-close" onClick={() => setOpen(false)} aria-label="Fermer">×</button>
        <p>Assigner à {displayLabel}</p><h2 id={`assignment-title-${student.id}`}>Quel type d’activité voulez-vous proposer?</h2>
        <div className="student-assignment-choices">
          <Link href={`/teacher/activities/new?${revisionParams.toString()}`}><strong>Activité de révision</strong><span>Pratiquer des connaissances et répondre à des questions.</span><em>Choisir →</em></Link>
          <Link href={`/teacher/activities/intellectual-operations?${guidanceParams.toString()}`}><strong>Guidage d’une opération intellectuelle</strong><span>Apprendre une démarche pas à pas avec Socrato.</span><em>Choisir →</em></Link>
        </div>
      </section>
    </div> : null}
  </>;
  ...
  return <main className="teacher-dashboard teacher-group-detail-page">
    <aside className="teacher-sidebar" aria-label="Navigation enseignante">
      ...
      <nav aria-label="Navigation principale">
        <Link className="sidebar-nav-tile teacher-home-link" href="/"><span>Page d’accueil</span><span aria-hidden="true">→</span></Link>
      ...
      <div className="group-detail-main">
        ...
        <section className="group-students-card" aria-labelledby="group-students-title">
          <div className="group-students-heading"><div><h2 id="group-students-title">Élèves du groupe</h2><p>Suivi synthétique pour cette activité.</p></div><div className="group-filters" aria-label="Filtres des élèves">
            {displayData.source === "server" ? <><Link className="group-access-codes-link" href={`/teacher/groups/${encodeURIComponent(displayData.groupId)}/codes`}>+ Ajouter un élève</Link><Link className="group-access-codes-link" href={`/teacher/groups/${encodeURIComponent(displayData.groupId)}/codes`}>Voir les codes d’accès</Link></> : null}
            <div className="priority-filter" role="group" aria-label="Filtrer selon la priorité"><button type="button" aria-pressed={priorityFilter === "all"} onClick={() => setPriorityFilter("all")}>Tous</button><button type="button" aria-pressed={priorityFilter === "medium"} onClick={() => setPriorityFilter("medium")}>À surveiller</button><button type="button" aria-pressed={priorityFilter === "high"} onClick={() => setPriorityFilter("high")}>Prioritaires</button></div>
          ...
          {visibleStudents.length ? <div className="group-students-table-wrap"><table><thead><tr><th scope="col">Élève</th><th scope="col">État de l’activité</th><th scope="col">Priorité</th><th scope="col">Raison du suivi</th><th scope="col">Bilan</th><th scope="col">Assigner une activité</th></tr></thead><tbody>{visibleStudents.map((student) => <tr key={student.id}><th scope="row" data-label="Élève">{student.studentDetailHref ? <Link className="group-student-name" href={student.studentDetailHref}>{student.displayLabel}</Link> : student.displayLabel}</th><td data-label="État de l’activité"><span className={`activity-state activity-state--${student.activityState}`}>{TEACHER_GROUP_STATE_LABELS[student.activityState]}</span>{student.activityState === "in_progress" ? <small className="student-progress-value">{student.progressPercentage ?? 0} % complété</small> : null}</td><td data-label="Priorité"><span className={`student-priority student-priority--${student.priority}`}>{student.priority === "high" ? "Priorité élevée" : student.priority === "medium" ? "À surveiller" : "Suivi normal"}</span></td><td data-label="Raison du suivi">{student.mainDifficulty}</td><td data-label="Bilan"><StudentDetailAction student={student} /></td><td data-label="Assigner une activité">{displayData.source === "server" ? <StudentConsolidationAction groupId={displayData.groupId} student={student} /> : null}</td></tr>)}</tbody></table></div> : <div className="group-filter-empty" role="status"><strong>Aucun élève ne correspond à ces filtres.</strong><p>Modifiez la priorité ou l’état sélectionné.</p></div>}
        </section>
      </div>
    </section>
  </main>;
