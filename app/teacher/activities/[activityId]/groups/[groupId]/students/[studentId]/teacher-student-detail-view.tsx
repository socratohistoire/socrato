...
import { applyLocalActivityToStudentDetail } from "@/lib/local-published-activities";
import { createTeacherStudentDetailViewModel } from "@/lib/teacher-student-detail";
import { LOCAL_STUDENT_ID } from "@/lib/academic-context";
...
export function TeacherStudentDetailView({ data }: { data: TeacherStudentDetailViewModel }) {
  ...
  useEffect(() => {
    if (!/^activity-local-[0-9]+$/.test(data.activityId) || data.studentId !== LOCAL_STUDENT_ID) return;
    const repository = createConfiguredDataRepository(window.localStorage);
    let active = true;
    void Promise.all([repository.listPublishedActivities(), repository.listStudentOutcomes()]).then(([activities, outcomes]) => {
      const activity = activities.find(({ id }) => id === data.activityId);
      const outcome = outcomes[data.activityId];
      if (active && activity && outcome) setDisplayData(createTeacherStudentDetailViewModel(applyLocalActivityToStudentDetail(data, activity, outcome)));
    });
    return () => { active = false; };
  }, [data]);
  return <TeacherStudentDetailContent data={displayData} />;
}

function TeacherStudentDetailContent({ data }: { data: TeacherStudentDetailViewModel }) {
  const consolidationSource = data.consolidationProgress.source === "teacher_assigned" ? "Assignée par l’enseignant" : "Proposée par Socrato";
  return <main className="teacher-dashboard teacher-group-detail-page teacher-student-detail-page">
    <aside className="teacher-sidebar" aria-label="Navigation enseignante">
      ...
      <nav aria-label="Navigation principale"><Link className="sidebar-nav-tile teacher-space-link" href={data.teacherReturnHref}><span className="sidebar-nav-icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M4 19V8l8-5 8 5v11M9 19v-6h6v6" /></svg></span><span>Espace enseignant</span><span aria-hidden="true">→</span></Link><TeacherGroupsDisclosure groups={data.groups} /><Link className="sidebar-create-action" href="/teacher/activities/new" aria-label="Créer une activité"><svg className="sidebar-create-icon" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M13 5h27l11 11v40H13z" /><path d="M40 5v12h11M21 27h21M21 35h17M21 43h13" /><circle cx="15" cy="48" r="10" /><path d="M15 42v12M9 48h12" /></svg><span>Créer une activité</span><span className="sidebar-create-divider" aria-hidden="true" /><span className="sidebar-create-arrow" aria-hidden="true">→</span></Link></nav>
    ...
    <section className="teacher-content group-detail-content" aria-labelledby="student-detail-title">
      ...
      <div className="student-detail-main">
        ...
        <div className="student-detail-second-row">
          <section className="student-detail-card student-pedagogical-card" aria-labelledby="student-pedagogical-title"><h2 id="student-pedagogical-title">Bilan pédagogique</h2><div className={`teacher-consolidation-progress teacher-consolidation-progress--${data.consolidationProgress.state}`}><span>Évolution après consolidation</span><h3>{data.consolidationProgress.previousLevel} <span aria-hidden="true">→</span> {data.consolidationProgress.currentLevel}</h3><p>{data.consolidationProgress.observation}</p><small>{consolidationSource} · Terminée le {data.consolidationProgress.completedAt}</small></div><dl><div className="student-pedagogical-item student-pedagogical-item--strength"><dt><PedagogicalIcon kind="strength" label="Point fort" /><span>Point fort actuel</span></dt><dd>{data.pedagogicalSummary.strength}</dd></div><div className="student-pedagogical-item student-pedagogical-item--difficulty"><dt><PedagogicalIcon kind="difficulty" label="Difficulté principale" /><span>Difficulté actuelle</span></dt><dd>{data.pedagogicalSummary.mainDifficulty}</dd></div><div className="student-pedagogical-item student-pedagogical-item--path"><dt><PedagogicalIcon kind="path" label="Piste de consolidation" /><span>Prochaine étape</span></dt><dd>{data.pedagogicalSummary.consolidationPath}</dd></div></dl><button type="button" className="student-consolidation-action" disabled aria-disabled="true" title="Fonction à venir">Créer une activité de consolidation <span aria-hidden="true">→</span><span className="sr-only"> — Fonction à venir</span></button></section>
