...
test("préserve le mode réduit et le sélecteur de thème", () => {
  ...
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch\{[^}]*gap:2px[^}]*min-height:44px[^}]*padding:2px[^}]*border:1px solid[^}]*box-shadow:none/);
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch button\{[^}]*min-width:54px[^}]*min-height:44px[^}]*background:transparent[^}]*font-size:\.64rem/);
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch button\[aria-pressed="true"\]\{background:var\(--teacher-theme-toggle-active-bg\);color:var\(--teacher-theme-toggle-active-text\);font-weight:700\}/);
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch button:focus-visible\{outline:2px solid var\(--teacher-theme-toggle-active-text\);outline-offset:-3px\}/);
...
test("centre un en-tête contextuel compact et responsive", () => {
  ...
  assert.match(cssSource, /\.teacher-brand-lockup\{min-width:0;height:auto;min-height:0;align-items:center;justify-content:flex-start;align-content:initial;gap:14px;padding-top:16px\}/);
  assert.match(cssSource, /\.teacher-brand-symbol,\.teacher-brand-copy\{flex:0 0 auto;margin:0\}/);
  assert.match(cssSource, /\.teacher-brand-copy\{align-items:center;gap:6px\}/);
  assert.match(cssSource, /\.teacher-brand strong\{font-size:1\.8125rem;font-weight:700[^}]*letter-spacing:\.035em[^}]*white-space:nowrap\}/);
  assert.match(cssSource, /\.teacher-brand small\{font-size:1\.03125rem;font-weight:400[^}]*white-space:nowrap\}/);
  assert.match(cssSource, /\.teacher-brand-symbol\{width:76px;height:76px;min-height:76px;max-height:76px;aspect-ratio:1;display:grid;place-items:center;margin:0;padding:0;overflow:hidden;flex:0 0 76px\}/);
  ...
  assert.match(cssSource, /@media\(max-width:1100px\)[\s\S]*\.teacher-brand-symbol\{width:60px;height:60px;min-height:60px;max-height:60px;flex-basis:60px\}/);
  assert.match(cssSource, /@media \(max-width:980px\)[\s\S]*\.teacher-brand\{[^}]*contain:none[^}]*text-align:left\}[^]*\.teacher-brand-lockup\{flex-direction:row;gap:8px;padding-top:0\}[^]*\.teacher-brand-copy\{align-items:flex-start;margin-top:0\}[^]*\.teacher-brand-symbol\{width:48px;height:48px;min-height:48px;max-height:48px;flex-basis:48px\}/);
  ...
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch button\{[^}]*min-width:54px[^}]*min-height:44px[^}]*font-size:\.64rem/);
  ...
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch button\[aria-pressed="true"\]\{background:var\(--teacher-theme-toggle-active-bg\);color:var\(--teacher-theme-toggle-active-text\);font-weight:700\}/);
  assert.match(cssSource, /\.teacher-dashboard \.theme-switch button\[aria-pressed="true"\]\{[^}]*background:var\(--teacher-theme-toggle-active-bg\)/);
